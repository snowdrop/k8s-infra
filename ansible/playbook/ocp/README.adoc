...
The default installation process uses the controller as the installation 
 ...
 process can be executed from a temporary bootstrap host.

To use the temporary boostrap host first create it. The name of this VM can 
 be any name but we suggest using the name of the cluster as prefix.
...
After creating the bootstrap server execute the steps provided in the <<deploy-ocp-on-rhos>> section. 

Finally after the OCP cluster is deployed you may remove the bootstrap server.
