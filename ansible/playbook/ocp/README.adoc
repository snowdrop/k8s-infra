...
First obtain the OCP pull secret which can be obained from https://console.redhat.com/openshift/install/pull-secret. 
 This information will be added to the `install-config.yaml` and used in the OCP 
...
----
{
  "auths": {
    ...
    ,"quay.io": {"auth": "xxxxxxxxxxxxx", "email": "antcosta@redhat.com"}
...
For the sake of example the `OCP_PULL_SECRET` environment variable will be used as container for this information.
