...
* Set up an Ingress
controller to route the traffic. 

The bash script has been developed supporting the installation of a docker registry according to the following options: 

* Unsecure: plain HTTP
* Secured: HTTP/HTTPS endpoints available
...
[source,bash]
----
./kind/deploy.sh command [parameters,...]
----
...
Deploying a kind cluster is as simple as executing the following command.

.kind deploy statement
[source,bash]
----
curl -s -L "https://raw.githubusercontent.com/snowdrop/k8s-infra/main/kind/deploy.sh" | bash -s install --ingress nginx --cluster-name twm --provider podman --port-map "30000:30000,31000:31000"
----
...
[source,bash]
----
curl -s -L "https://raw.githubusercontent.com/snowdrop/k8s-infra/main/kind/deploy.sh" | bash -s remove --cluster-name twm
----
