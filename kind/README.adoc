...
For that purpose, a bash script has been developed: `./kind.sh`, which can be accessed using `curl -s -L "https://raw.githubusercontent.com/snowdrop/k8s-infra/main/kind/kind.sh" | bash -s ...`
