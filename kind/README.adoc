...
For that purpose, a bash script has been developed: `./kind.sh`, which can be acessed using curl ``
and will let to create or delete a kubernetes cluster using the ttps://kind.sigs.k8s.io/[kind] tool.

The container registry is started as a `container`, its network shared with the host and can be deployed using the mode:

* Unsecure: plain HTTP without user authentication
